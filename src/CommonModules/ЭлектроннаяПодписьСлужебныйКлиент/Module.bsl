...
Процедура ПослеПодключенияКомпоненты(Подключено, Контекст) Экспорт
	...
	Если Подключено Тогда
		...
		Исключение
			...
			ЗавершитьОперациюСОшибкой(
				Контекст,
				НСтр("ru='Ошибка инициализации компоненты XMLDSig.';uk='Помилка ініціалізації компоненти XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	ИначеЕсли Контекст.УстанавливатьКомпоненту Тогда
		...
		ТекстВопроса = НСтр("ru='Для подписания XML необходима установка компоненты XMLDSig.
                                  ...
                                  |;uk='Для підписання XML необхідне встановлення компоненти XMLDSig.
                                  |Перейти до встановлення даної компоненти?'");
		ПоказатьВопрос(
...
Процедура НачатьПодписаниеСообщенияSOAP(ИнформацияМодуляКриптографии, Контекст) Экспорт
	...
	Если ТипКриптопровайдера = Неопределено Тогда
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			СтрШаблон(
				НСтр("ru='Не удалось определить тип криптопровайдера %1';uk='Не вдалося визначити тип криптопровайдера %1'"), ИнформацияМодуляКриптографии.Имя)
...
Процедура Проверка_ПослеВыполненияVerifySign(ПодписьВерна, Параметры, Контекст) Экспорт
	...
	Если Не ПодписьВерна Тогда
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ru='Подпись не верна (SignatureValue не корректно.';uk='Підпис не вірний (SignatureValue не коректно.'"));
