...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Параметры.Ключ) Тогда
		ТекстОшибки = НСтр("ru='Общая форма ""Предупреждение безопасности"" является вспомогательной и открывается из служебных механизмов программы.';uk='Загальна форма ""Попередження безпеки"" є допоміжною і відкривається з службових механізмів програми.'");
...
Процедура ПредложитьПерезапуск()
	...
	ТекстВопроса = НСтр("ru='Для применения изменений в правах доступа требуется перезапустить программу.';uk='Для застосування змін в правах доступу потрібно перезапустити програму.'");
