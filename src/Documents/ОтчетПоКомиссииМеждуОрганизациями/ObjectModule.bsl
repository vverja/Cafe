...
Процедура ЗаполнитьВидыЗапасов(Отказ)
    ...
    КонецЕсли;
	Если ВидыЗапасов.Найти(Справочники.ВидыЗапасов.ПустаяСсылка(), "ВидЗапасовКомитента") <> Неопределено Тогда
		ДозаполнитьВидыЗапасов(МенеджерВременныхТаблиц, Отказ);
	КонецЕсли;
...
Функция ТоварыРеализованныеКомиссионерами()
	
	Строки = ВидыЗапасов.Выгрузить(, "АналитикаУчетаНоменклатуры, НомерГТД, Количество");
	Строки.Свернуть("АналитикаУчетаНоменклатуры, НомерГТД", "Количество");
	Запрос = Новый Запрос("
	...
	|		ПО ОтчетПоКомиссии.Ссылка = Движения.Регистратор
	...
	|ГДЕ
	|	Движения.Период <= &ДатаОстатков
	|	И Движения.Активность
	|	И Движения.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|	И ОтчетПоКомиссии.Организация = &Организация
	|	И ОтчетПоКомиссии.Комиссионер = &Комиссионер
	|	И ОтчетПоКомиссии.Ссылка <> &Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	Строки.АналитикаУчетаНоменклатуры,
	|	Движения.ВидЗапасов,
...
Процедура ДозаполнитьВидыЗапасов(ВременныеТаблицы, Отказ)
	
	Если ВременныеТаблицы.Таблицы.Найти("Остатки") <> НЕОПРЕДЕЛЕНО Тогда
	...
	// Заполним вид запасов комитента по раннее оформленным отчетам по комиссии между организациями.
	РеализованныеТовары = ТоварыРеализованныеКомиссионерами();
	...
	Для Каждого СтрокаОстатка Из РеализованныеТовары Цикл
		ЗаполнитьЗначенияСвойств(СтруктураПоиска, СтрокаОстатка);
		Для Каждого СтрокаЗапасов Из ВидыЗапасов.НайтиСтроки(СтруктураПоиска) Цикл
			Если СтрокаЗапасов.Количество = 0 ИЛИ ЗначениеЗаполнено(СтрокаЗапасов.ВидЗапасовКомитента) Тогда
				Продолжить;
			КонецЕсли;
			Количество = ?(СтрокаЗапасов.Количество < 0, -СтрокаЗапасов.Количество, СтрокаЗапасов.Количество);
			Знак = ?(СтрокаЗапасов.Количество < 0, -1, 1);
			Если СтрокаОстатка.Количество >= Количество Тогда
				СтрокаЗапасов.ВидЗапасовКомитента = СтрокаОстатка.ВидЗапасов;
				Если Знак > 0 Тогда
					СтрокаОстатка.Количество = СтрокаОстатка.Количество - Количество;
				Иначе
					СтрокаОстатка.Количество = СтрокаОстатка.Количество + Количество;
			...
			ИначеЕсли СтрокаОстатка.Количество < Количество Тогда
				НоваяСтрока = ВидыЗапасов.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаЗапасов);
				НоваяСтрока.ВидЗапасовКомитента = СтрокаОстатка.ВидЗапасов;
				НоваяСтрока.СуммаСНДС = Окр(СтрокаОстатка.Количество * СтрокаЗапасов.СуммаСНДС / СтрокаЗапасов.Количество, 2, 1);
				НоваяСтрока.СуммаНДС = Окр(СтрокаОстатка.Количество * СтрокаЗапасов.СуммаНДС / СтрокаЗапасов.Количество, 2, 1);
				НоваяСтрока.СуммаВознаграждения = Окр(СтрокаОстатка.Количество * СтрокаЗапасов.СуммаВознаграждения / СтрокаЗапасов.Количество, 2, 1);
				НоваяСтрока.СуммаНДСВознаграждения = Окр(СтрокаОстатка.Количество * СтрокаЗапасов.СуммаНДСВознаграждения / СтрокаЗапасов.Количество, 2, 1);
				НоваяСтрока.КоличествоУпаковок = Окр(СтрокаОстатка.Количество * СтрокаЗапасов.КоличествоУпаковок / СтрокаЗапасов.Количество, 2, 1);
				НоваяСтрока.Количество = СтрокаОстатка.Количество * Знак;
	...
	МассивПустойВидЗапасовКомитента = ВидыЗапасов.НайтиСтроки(Новый Структура("ВидЗапасовКомитента", Справочники.ВидыЗапасов.ПустаяСсылка()));
	Если КонтролироватьОстаткиТоваровОрганизаций И МассивПустойВидЗапасовКомитента.Количество() > 0 Тогда 
		Отказ = Истина;
		Для Каждого СтрокаТаблицы Из МассивПустойВидЗапасовКомитента Цикл
			Аналитика = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаТаблицы.АналитикаУчетаНоменклатуры, "Номенклатура, Характеристика");
			ЕдиницаИзмерения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Аналитика.Номенклатура, "ЕдиницаИзмерения");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Номенклатура: %1 
                |Сторно отчета по комиссии превышает остаток товара комитента %2, возвращенного от комиссионера %3, на %4 %5'
                |;uk='Номенклатура: %1 
                |Сторно звіту комісії перевищує залишок товару комітента %2, повернутого від комісіонера %3, на %4 %5'"),
				НоменклатураКлиентСервер.ПредставлениеНоменклатуры(Аналитика.Номенклатура, Аналитика.Характеристика),
				Организация,
				Комиссионер,
				?(СтрокаТаблицы.Количество < 0, -СтрокаТаблицы.Количество, СтрокаТаблицы.Количество),
				ЕдиницаИзмерения);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ТекстСообщения,
				Ссылка);
	...
	Иначе
		...
		Для Каждого СтрокаТаблицы Из МассивПустойВидЗапасовКомитента Цикл
			НоваяСтрока.ВидЗапасовКомитента = ВидЗапасовПоУмолчанию;
		КонецЦикла;
	КонецЕсли;
