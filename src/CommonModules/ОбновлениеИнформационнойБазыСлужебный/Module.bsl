			...
			Исключение
				...
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='У обработчика не правильно заполнено свойство Версия: ""%1"".
                               ...
                               |;uk='У обробника не правильно заповнена властивість Версія: ""%1"".
		...
		Если Обработчик.РежимВыполнения <> ""
			...
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='У обработчика ""%1"" не правильно заполнено свойство РежимВыполнения.
                           ...
                           |;uk='У обробника ""%1"" не правильно заповнена властивість РежимВыполнения.
...
Функция ВыполнитьОтложенныйОбработчикОбновления(СведенияОбОбновлении, ПараметрыОбновления = Неопределено)
    ...
    КонецЕсли;
    ЕстьОбработчикиПодредакций = Ложь;
	Для Каждого СтрокаОбработчик Из ТекущийЦиклОбновления.Обработчики Цикл
        Если СтрНайти(СтрокаОбработчик, "НачалоПодредакции") > 0 Тогда
            ЕстьОбработчикиПодредакций = Истина;
            Прервать;
        КонецЕсли;
    КонецЦикла;    
    ПараметрыОбновления.Вставить("ЕстьОбработчикиПодредакций", ЕстьОбработчикиПодредакций);
...
Процедура ЗапуститьПроцедуруОбработкиДанных(ОбработчикОбновления, ПараметрыОбновления)
	...
	Попытка
        ...
        Если НЕ ПараметрыОбновления.ЕстьОбработчикиПодредакций Тогда
            Параметры.Вставить("НеЗаполнятьДанныеВОбработчикеПодредакции", Истина);
        КонецЕсли;    
...
Процедура ЗаполнитьДанныеДляПараллельногоОтложенногоОбновления(СведенияОбОбновлении, Параметры)
	...
	Для Каждого СтрокаБиблиотека Из СведенияОбОбновлении.ДеревоОбработчиков.Строки Цикл
		...
		Для Каждого СтрокаВерсия Из СтрокаБиблиотека.Строки Цикл
            Для Каждого Обработчик Из СтрокаВерсия.Строки Цикл
                Если СтрНайти(Обработчик.ИмяОбработчика, "НачалоПодредакции") > 0 Тогда
                    ЕстьОбработчикиПодредакций = Истина;
                    Прервать;
                КонецЕсли;
            КонецЦикла;
            Если ЕстьОбработчикиПодредакций Тогда
                Прервать;
            КонецЕсли; 
        КонецЦикла;    
		...
		Для Каждого СтрокаВерсия Из СтрокаБиблиотека.Строки Цикл
            Для Каждого Обработчик Из СтрокаВерсия.Строки Цикл
                ...
                Если ЕстьОбработчикиПодредакций И НЕ Обработчик.НеЗаполнятьДанныеВОбработчикеПодредакции Тогда
                    Продолжить;
                КонецЕсли; 
