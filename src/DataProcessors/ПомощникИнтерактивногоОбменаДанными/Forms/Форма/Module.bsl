...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если МассивУзлов.Найти(Объект.УзелИнформационнойБазы) = Неопределено Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Для выбранного узла сопоставление данных не предусмотрено.';uk=' Для обраного вузла співставлення даних не передбачене.'"),,,, Отказ);
		Возврат;
...
Процедура ОбработчикОжиданияФоновогоЗадания()
	...
	Иначе // Завершено, ЗавершеноАварийно
		...
		Если Состояние <> "Завершено" Тогда
			...
			ДлительнаяОперацияЗавершенаСОшибкой = Истина;
		Иначе
			// Чтение результата завершения.
			Результат = Новый Структура("Отказ", Ложь);
			ФоновоеЗаданиеЗагрузитьРезультат(Результат);
			ДлительнаяОперацияЗавершенаСОшибкой = Результат.Отказ;
		КонецЕсли;
...
Процедура ФоновоеЗаданиеЗагрузитьРезультат(Результат)
	Если АдресВременногоХранилища = Неопределено Тогда
		Возврат;
	КонецЕсли;
	РезультатВыполненияВФоне = ПолучитьИзВременногоХранилища(АдресВременногоХранилища);
	Если РезультатВыполненияВФоне = Неопределено Тогда
		Возврат;
	ИначеЕсли РезультатВыполненияВФоне.Свойство("Отказ") Тогда
