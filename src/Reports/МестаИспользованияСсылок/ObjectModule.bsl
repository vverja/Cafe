	...
	Если ТипЗнч(НаборСсылок) = Тип("СписокЗначений") Тогда
		...
		Если ВсегоСсылок >= 0 Тогда
			...
			Если ОдинаковыйТип Тогда
				Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Места использования %1';uk='Місця використання %1'"), НаборСсылок[0].Значение.Метаданные().Представление());
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
