...
Функция ПравилаКонвертацииСовместимыСТекущейВерсией(ИмяПланаОбмена, ОписаниеОшибки, ДанныеОПравилах)
	...
	Если РезультатСравнения <> 0 Тогда
		...
		Если РезультатСравнения < 0 Тогда
			...
			ТекстОшибки = НСтр("ru='Синхронизация данных может работать некорректно, так как загружаемые правила предназначены для предыдущей версии программы ""%1"" (%2). Рекомендуется использовать правила из конфигурации или загрузить комплект правил, предназначенный для текущей версии программы (%3).';uk='Синхронізація даних може працювати некоректно, так як правила, що завантажуються призначені для попередньої версії програми ""%1"" (%2). Рекомендується використовувати правила з конфігурації або завантажити комплект правил, призначений для поточної версії програми (%3).'");
			ВидОшибки = "УстаревшаяВерсияКонфигурации";
			
		Иначе
			
			ТекстОшибки = НСтр("ru='Синхронизация данных может работать некорректно, так как загружаемые правила предназначены для более новой версии программы ""%1"" (%2). Рекомендуется обновить версию программы или использовать комплект правил, предназначенный для текущей версии программы (%3).';uk='Синхронізація даних може працювати некоректно, так як правила, що завантажуються призначені для більш нової версії програми ""%1"" (%2). Рекомендується оновити версію програми або використовувати комплект правил, призначений для поточної версії програми (%3).'");
