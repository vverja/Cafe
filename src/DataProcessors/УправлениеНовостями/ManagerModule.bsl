				...
				Если ТипЗнч(СписокВсехДанных.channel) = ТипСписокXDTO Тогда // Правильно сработает, даже если передали только одно значение.
					...
					Для Каждого ТекущийЭлементСписка Из СписокВсехЗагруженныхСерверныхЛентНовостей Цикл
						Если ТекущийЭлементСписка.Пометка = Ложь Тогда
							...
							ТекстСообщения = НСтр("ru='Лента новостей %ЛентаНовостейНаименование% помечена на удаление, т.к. она была отключена на сервере';uk='Стрічка новин %ЛентаНовостейНаименование% позначена на вилучення, т.я. вона була відключена на сервері'");
...
Процедура ПарсингХМЛФайлаНовостей(ОбъектХДТО, ФорматНовостей, ЛентаНовостей)
	...
	Иначе
		...
		РезультатЗагрузкиНовостей = НСтр("ru='Новости не загружены, т.к. передан необрабатываемый формат ленты новостей.';uk='Новини не завантажені, тому що переданий необроблюваний формат стрічки новин.'");
