...
Процедура ЗарегистрироватьДанныеКОбработкеДляЗаполненияНомераПлатежногоДокумента(Параметры) Экспорт
	...
	ДопПараметрыОтметки.ПолноеИмяРегистра = ПолноеИмяРегистра();
	...
	|ГДЕ
//	|	ДанныеРегистра.НомерПлатежногоДокумента = """"
//	|	И 
	|	НЕ ДанныеРегистра.Регистратор ССЫЛКА Документ.КорректировкаРегистров";
...
Процедура ПерезаписатьВсеДвиженияПриОбновлении(Параметры) Экспорт
	...
	Пока ВыборкаПоРегистраторам.Следующий() Цикл
		...
		Попытка
			...
			ДенежныеСредстваСервер.ЗаполнитьНомерПлатежногоДокументаЕдиныйНалог(НаборЗаписей);
