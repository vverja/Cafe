...
Процедура ЗаполнитьКолонкиТаблицыИсходныхСсылок()
	...
	Если ТипыОбщихДанных.Найти(ТекущийОбъектМетаданных) <> Неопределено Тогда
		ПоляЕствественногоКлюча = ТехнологияСервисаИнтеграцияСБСП.МенеджерОбъектаПоПолномуИмени(ТекущийОбъектМетаданных.ПолноеИмя()).ПоляЕстественногоКлюча();
...
Процедура ЗаписатьИмяКолонкиИсходныхСсылок()
	...
	ВыгрузкаЗагрузкаДанныхСлужебный.ЗаписатьОбъектВФайл(ТекущееИмяКолонкиСИсходнойСсылкой, ИмяФайла);
