...
Процедура СписокПолучателейОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Не ПустаяСтрока(Текст) Тогда
		...
		ПодстрокиВведенногоТекста = СтрРазделить(Текст, ";", Ложь);
		ПодстрокиДанныхОбъекта    = СтрРазделить(Объект[ИмяЭлемента], ";", Ложь);
...
Процедура ОтправитьПереслатьВыполнить(Команда)
	...
	Иначе
		...
		Если Объект.ПолучателиПисьма.Количество() = 0
			И Объект.ПолучателиСкрытыхКопий.Количество() = 0 Тогда
