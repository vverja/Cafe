	...
	Если СтруктураВыбора.СОшибками.Количество > 0 Тогда
		...
		ИначеЕсли СтруктураВыбора.Выбрано.Количество >= 6 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |;uk='Додано звітів: %1 з %2,
                |Не додано через помилки: %3'"),
...
Процедура ПроверитьРассылкуПослеВыбораПолучателей(РезультатВыбора, ПараметрыДоставки) Экспорт
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		ПараметрыДоставки.Вставить("ШаблонТемы",    НСтр("ru='Тестовое сообщение конфигурации BAS';uk='Тестове повідомлення конфігурації BAS'"));
		ПараметрыДоставки.Вставить("ШаблонТекста",  НСтр("ru='Это сообщение отправлено системой рассылок конфигурации BAS.';uk='Це повідомлення надіслано системою розсилок конфігурації BAS.'") + Символы.ПС + Кэш.ЗаголовокСистемы);
