...
Функция TestConnection(ИмяПланаОбмена, КодУзла, Результат)
	...
	Если ПланыОбмена[ИмяПланаОбмена].НайтиПоКоду(КодУзла).Пустая() Тогда
		Результат = НСтр("ru='Возможность синхронизации данных отключена администратором приложения в Интернете.';uk='Можливість синхронізації даних відключена адміністратором прикладної програми в Інтернеті.'");
		Возврат Ложь;
	КонецЕсли;
