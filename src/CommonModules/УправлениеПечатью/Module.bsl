...
Функция МакетПечатнойФормы(ПолныйПутьКМакету, Знач КодЯзыка = "") Экспорт	
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Макет ""%1"" не найден. Операция прервана.';uk='Макет ""%1"" не знайдено. Операція перервана.'"), ПолныйПутьКМакету);
	КонецЕсли;
