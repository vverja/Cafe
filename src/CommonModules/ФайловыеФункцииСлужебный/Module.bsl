...
Процедура ПроверитьИсключениеHTTP1С(Ответ, АдресСервера)
	Если (Ответ.КодСостояния >= 400) И (Ответ.КодСостояния <= 599) Тогда
		ШаблонОшибки = НСтр("ru='Не удалось синхронизировать файл по адресу %2, т.к. сервер вернул HTTP код: %1.';uk='Не вдається синхронізувати файл за адресою %2, т. я. сервер повернув HTTP код: %1.'");
