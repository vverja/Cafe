...
Функция ПодставитьДомен(Текст, Знач ДоменнаяЗона = Неопределено) Экспорт
	...
	Если ДоменнаяЗона = 1 Тогда
		Результат = СтрЗаменить(Текст, "webits-info@1c.ru", "webits-info@bas-soft.eu");
		Результат = СтрЗаменить(Текст, "webits-info@1c.ua", "webits-info@bas-soft.eu");
		Результат = СтрЗаменить(Текст, ".1c.ru", ".bas-soft.eu");
		Результат = СтрЗаменить(Текст, ".1c.eu", ".bas-soft.eu");
		Возврат Результат;
	Иначе
		Возврат Текст;
	КонецЕсли;
...
Функция ПользовательскоеПредставлениеНомераВерсии(Версия, СокращатьРазрядностьВерсии = Ложь) Экспорт
	...
	Иначе
		...
		Версия = ВнутреннееПредставлениеНомераВерсии(Версия);
	...
	Для Каждого ТекущийШаг Из КонтекстВыполнения.ЛогШаговВыполнения Цикл
		...
		ИначеЕсли ВариантФормата = "ПодробноПоШагам" Тогда
			...
			ШаблонШага = НСтр("ru='%1Шаг: %2
                |%1Длительность (мс): %3 (%4 - %5)
                |%1Результат выполнения: %6
                |'
                |;uk='%1крок: %2
                |%1Длительность (мс): %3 (%4 - %5)
                |%1Результат виконання: %6
                |'");
			Результат = Результат
				+ "#"
				+ СтрШаблон(
					ШаблонШага,
						...
						ТекущийШаг.ВремяОкончания,
						ТекущийШаг.ВремяНачала,
...
Процедура СлужебнаяПроверитьURLДоступен(URL, ИмяОшибки, СообщениеОбОшибке, ИнформацияОбОшибке) Экспорт
