...
Функция ПроверитьПодпись(Знач КонвертSOAP, ДанныеАлгоритмаПодписания, МенеджерКриптографии) Экспорт
	...
	Иначе
		...
		Иначе
			ВызватьИсключение НСтр("ru='Подпись не верна (SignatureValue не корректно.';uk='Підпис не вірний (SignatureValue не коректно.'");
		КонецЕсли;
...
Функция ТипКриптопровайдера(МенеджерКриптографии)
	...
	Если ТипКриптопровайдера = Неопределено Тогда
		ВызватьИсключение СтрШаблон(НСтр("ru='Не удалось определить тип криптопровайдера %1';uk='Не вдалося визначити тип криптопровайдера %1'"), ИмяКриптопровайдера);
	КонецЕсли;
