...
Процедура ОпределитьНастройки(Настройки, ИдентификаторНастройки = "") Экспорт
	...
	Настройки.Вставить("ЗаголовокКомандыДляСозданияНовогоОбменаДанными", НСтр("ru='РИБ с фильтрами';uk='РІБ з фільтрами'"));
	Настройки.Вставить("ЗаголовокПомощникаСозданияОбмена",               НСтр("ru='Настройка распределенной информационной базы с отборами';uk='Налаштування розподіленої інформаційної бази з відборами'"));
	Настройки.Вставить("ЗаголовокУзлаПланаОбмена",                       НСтр("ru='Узел РИБ с фильтрами';uk='Вузол РІБ з фільтрами'"));
...
Функция ОписаниеОграниченийПередачиДанных(НастройкаОтборовНаУзле, ВерсияКорреспондента, ИдентификаторНастройки = "") Экспорт
	...
	Если КоллекцияЗначений.Количество() > 0 Тогда
		...
		ТекстОписания = ТекстОписания + НСтр("ru='Подразделения, по которым будут отправлены данные: %ПредставлениеКоллекции%;';uk='Підрозділи, за якими будуть відправлені дані: %ПредставлениеКоллекции%;'");
		ТекстОписания = СтрЗаменить(ТекстОписания, "%ПредставлениеКоллекции%", ПредставлениеКоллекции);
	Иначе
		ТекстОписания = ТекстОписания + НСтр("ru='Подразделения для фильтрации отправляемых данных не указаны';uk='Підрозділи для фільтрації даних, що відправляються, не зазначені'");
	КонецЕсли;
	...
	Если НастройкаОтборовНаУзле.ИспользоватьОтборПоВидамЦен Тогда
		...
		ПредставлениеКоллекции = СокращенноеПредставлениеКоллекцииЗначений(КоллекцияЗначений);
		ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ru='Отправлять данные о ценах номенклатуры по видам цен: %ПредставлениеКоллекции%;';uk='Надсилати дані про ціни номенклатури по видах цін: %ПредставлениеКоллекции%;'");
		ТекстОписания = СтрЗаменить(ТекстОписания, "%ПредставлениеКоллекции%", ПредставлениеКоллекции);
		
	Иначе
		
		ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ru='Отправлять информацию о всех ценах номенклатуры';uk='Відправляти інформацію про всі ціни номенклатури'");
	...
	Если НастройкаОтборовНаУзле.ВыгружатьИнформативныеОстатки = 2 Тогда
		...
		ПредставлениеКоллекции = СокращенноеПредставлениеКоллекцииЗначений(КоллекцияЗначений);
		ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ru='Отправлять информативные остатки по складам: %ПредставлениеКоллекции%;';uk='Відправляти інформативні залишки по складах: %ПредставлениеКоллекции%;'");
		ТекстОписания = СтрЗаменить(ТекстОписания, "%ПредставлениеКоллекции%", ПредставлениеКоллекции);
		
	ИначеЕсли НастройкаОтборовНаУзле.ВыгружатьИнформативныеОстатки = 1 Тогда
		
		ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ru='Отправлять информативные остатки по всем складам';uk='Відправляти інформативні залишки по всіх складах'");
		
	Иначе
		
		ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ru='Не отправлять информативные остатки по складам';uk='Не відправляти інформативні залишки по складах'");
